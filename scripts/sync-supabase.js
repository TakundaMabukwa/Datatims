...
function toJsonComparable(value) {
  ...
  if (Array.isArray(value)) {
    return value.map(toJsonComparable);
  }
...
async function syncTable({ supabase, table, sourceRows, mapRow, conflictColumn, label }) {
  ...
  const existingMap = new Map(
    existingRows.map(row => [normalizeText(row[conflictColumn]), row])
  );
  ...
  for (const rowChunk of chunk(toUpsert, BATCH_SIZE)) {
    await upsertBatch(supabase, table, rowChunk, conflictColumn);
  }
...
async function syncDriversTable({ supabase, sourceRows }) {
  const mappedRows = sourceRows
    .map(mapDriver)
  ...
  for (const rowChunk of chunk(toUpsert, BATCH_SIZE)) {
    await upsertBatch(supabase, 'drivers', rowChunk, 'driver_code');
  }
