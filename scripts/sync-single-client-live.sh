...
LOG_FILE="${LOG_FILE:-$ROOT_DIR/logs/sync-single-client.log}"
CLIENT_ID="${1:-}"
...
if [[ -z "$CLIENT_ID" ]]; then
  echo "Usage: bash ./scripts/sync-single-client-live.sh <client_id>" >&2
  exit 1
fi
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] sync-single-client starting"
...
cd "$ROOT_DIR"
DRY_RUN="$DRY_RUN_VALUE" node "$ROOT_DIR/scripts/sync-single-client.js" "$CLIENT_ID"
echo "[$(date '+%Y-%m-%d %H:%M:%S')] sync-single-client completed"
